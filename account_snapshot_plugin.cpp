...
public:
  ...
  void process_account_traces(const vector<action_trace> traces)
  {
    for (const auto &atrace : traces)
    {
      ...
      // process_account_traces(atrace.action_trace);
    }
  }
...
{
  cfg.add_options()("account-snapshot-file", bpo::value<std::string>()->composing(),
...
void account_snapshot_plugin::plugin_initialize(const variables_map &options)
{
  ...
  /*
    ...
    */
  my->applied_transaction_connection.emplace(chain.applied_transaction.connect([&](std::tuple<const transaction_trace_ptr &, const signed_transaction &> t) {
